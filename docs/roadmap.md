...
## Status / product decisions (2026-07-21)

- 公开查询 / Subject / Event / Statistics API 已落地；旧 Case 公开读带 Deprecation/Sunset。
- **API Key：决定不实现**（见 `AGENTS.md` Product decisions）。公开读靠限流；写操作靠 JWT + RBAC。
- **OpenAPI 自动生成：延后**；当前契约以 handler 路由、README 端点表与 `docs/api/*` 为准。
...
- 分页正常
- Rate Limit 生效
- API Key 能正确控制权限（**已决策：不实现 API Key 产品**）
...
- **主路径与验收矩阵已合入 main（2026-07-21，含 `feature/next-development` merge `90ef0c2` 及后续 smoke 修复）**；规划 Goal/Verification 原文未改。
- 已落地并有证据：Auth/Settings Provider、navigation registry、统一 `lib/api.ts`（页面无直连 token/fetch）、CSS token + Tailwind semantic colors、最小 UI 状态组件、角色/视口 Playwright **21/21**、生产 Compose/Nginx smoke、unit/typecheck/lint/build。
- 仍待加强（非阻塞主路径）：全站状态机完全一致、WCAG 系统审计、管理端表格密度/工具栏、完整暗色模式（Out of Scope）。
- 设计规范：`DESIGN.md`
- 设计规格：`docs/compose/specs/2026-07-20-dynamic-frontend-design.md`
- 实施计划：`docs/compose/plans/2026-07-20-dynamic-frontend.md`、`docs/compose/plans/2026-07-21-next-development.md`
...
**进度说明（非改 Goal）**：A–F 主交付已合入 main 并有自动化/e2e/smoke 证据。剩余为体验打磨与 a11y 深度验收，详见 `docs/frontend-gap-analysis.md`。
...
- **主路径 + 收束已合入 main（2026-07-21，Phase 13 tip 经 `49e4ddc` 合入；next-development `90ef0c2` + smoke 修复）**；规划 Goal/Verification 原文未改。
- 设计规格：`docs/compose/specs/2026-07-20-subject-event-governance-design.md`
- 实现差距：`docs/implementation-gap-analysis.md`
- 开发排序：`docs/compose/plans/2026-07-20-subject-event-governance-execution.md`、`docs/compose/plans/2026-07-21-next-development.md`
- 已验证：迁移 `000006`–`000011`、`UBS_<ULID>`、Account/Event、multipart 发布、链接证据、Local + MinIO/S3、Case 弃用头/Sunset、处罚与 Event 申诉主路径、demo captcha、归档 ZIP、`go test ./...`、前端 build/e2e、生产 Compose smoke。
- 有意保留至 2026-12-31：旧 Case/Submission 管理入口（兼容窗口，见 `docs/api/case-event-migration.md`）。
