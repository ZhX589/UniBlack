...
- **部分实施已合入 main（2026-07-21，merge `49e4ddc`）**；规划目标未改。
- 已落地进度：Auth/Settings Provider、角色过滤顶栏、管理 layout 守卫与侧栏、登录/注册 Link、构建通过。
- 仍待完成进度：design token 体系、统一 API client、Playwright 矩阵、完整响应式/无障碍验收。
- 设计规范：`DESIGN.md`
...
**进度说明（非改 Goal）**：B/C 的 session/settings 与 App Shell/管理布局已有可运行子集合入 main；A/D/E/F 及完整 Verification 矩阵仍待完成。详见 `docs/frontend-gap-analysis.md`。
...
- **主路径已合入 main（2026-07-21，merge `49e4ddc`，功能 tip `c6f6d42`）**；规划 Goal/Verification 原文未改。
...
- 已验证进度：迁移 000006–000010、public ID、Account/Event、multipart 多文件+文本同事务发布、处罚一次申诉/裁决、demo captcha、归档导入导出加固、动态 Auth Shell；main 上 `go test`/`go build`/`npm run build` 通过。
- 仍待完成进度：旧 Case API 弃用窗口、MinIO/S3 adapter、Phase 12 剩余 design token/Playwright。
