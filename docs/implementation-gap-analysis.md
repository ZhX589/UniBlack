...
> 实施更新：`feature/subject-event-governance` 已实现并验证 `000006`–`000009` 兼容迁移、`UBS_<ULID>`、Account/Event、真实 LocalStorage、受限 UTF-8 文本证据、默认发布、分级处罚、demo captcha、开发固定验证码、归档导出、冲突预览与确认导入。完整通用文件发布事务、处罚申诉、完整动态 Shell 与旧 Case API 弃用仍未完成。
...
| 导出/导入包 | 部分实现 | ZIP、manifest v1、README、SHA-256、账号/public ID 冲突预览与确认导入已 smoke | 导入来源记录、异步大包任务和回滚 UI 待补 |
...
| 前端动态化 | 未实现 | 约 23 处直接 fetch、14 处 localStorage、16 个内部 raw anchor | Phase 12 仍未实施，不能用现有控制台页面作为完成证据 |
| 测试 | 未实现 | 未见 frontend unit/e2e；后端功能测试不足 | 需要先补领域纯函数测试，再做集成和浏览器验收 |
| 部署可靠性 | 部分实现 | Docker 使用 `npm install`；前端 rewrite 依赖构建时环境；LocalStorage 不写盘 | Phase 12/13 目标所需的可复现构建和持久化存储尚未满足 |
...
```text
通用文件证据尚未随发布请求传入
  → 文本证据已有事务/补偿路径，但二进制文件仍要独立上传
  → 发布与存储的完整多文件原子体验尚未验收
```

已完成的领域迁移、demo 验证、处罚、文本证据与归档导入为后续两条链提供稳定基础。下一步应完成通用文件证据事务，最后扩展处罚申诉与动态管理 Shell。
